...
When("I click on the Pages tab - Scenario four", async function () {
  ...
  await takeKrakenScreenshot(this.driver, "Scenario4", "goToPageList");
});

When("I click on the New Page button", async function () {
  await clickNewPageButton(this.driver);
  await takeKrakenScreenshot(this.driver, "Scenario4", "goToCreatePage");
});

When(
  "I fill out a new page contents with title {kraken-string}",
  async function (page_title) {
    await fillNewPage(this.driver, page_title);
    await takeKrakenScreenshot(this.driver, "Scenario4", "createNewPage");
  }
);
...
Then(
  "I should see the new page published called {kraken-string}",
  async function (page_title) {
