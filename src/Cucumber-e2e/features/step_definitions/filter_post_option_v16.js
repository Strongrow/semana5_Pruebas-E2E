...
Given('Abrir la pagina de ghost v16', async function () {

    await testController.navigateTo(ghostPage.ghost.url());

});

When('Ingreso el usuario v16 {string}', async function (username) {
    await testController.typeText(ghostPage.ghost.loginUsername(), username);
});

When('Ingreso el password v16 {string}', async function (password) {
    await testController.typeText(ghostPage.ghost.loginPassword(), password);
});

When('Dar click en el boton Login v16', async function () {
    await testController.click(ghostPage.ghost.loginButton());
});


When('Ir a post v16', async function () {
...
When('Seleccionar un filtro al azar de los tipos de post v16', async function () {
    var numero_aleatorio = faker.number.int({ min: 0, max: 5 });

    console.log("valor del sub-menu a buscar... " + numero_aleatorio);

    const buttons1 = Selector('span').withText('All posts');
    ...
    if (numero_aleatorio == 4) {
        ...
        const buttons2 = Selector('[data-option-index="4"]');
        await testController.click(buttons2);
        await testController.wait(4000);
    }
    ...
    if (numero_aleatorio == 5) {
        ...
        const buttons2 = Selector('[data-option-index="5"]');
        await testController.click(buttons2);
        await testController.wait(4000);
    }
...
Then('Verificamos que no existan errorres v16', async function () {
