...
Given('Abrir la pagina de ghost v17', async function () {

    await testController.navigateTo(ghostPage.ghost.url());

});

When('Ingreso el usuario v17 {string}', async function (username) {
    await testController.typeText(ghostPage.ghost.loginUsername(), username);
});

When('Ingreso el password v17 {string}', async function (password) {
    await testController.typeText(ghostPage.ghost.loginPassword(), password);
});

When('Dar click en el boton Login v17', async function () {
    await testController.click(ghostPage.ghost.loginButton());
});


When('Ir a post v17', async function () {
...
When('Seleccionar un filtro al azar de los tipos de post All access v17', async function () {
    var numero_aleatorio = faker.number.int({ min: 0, max: 3 });

    console.log("valor del sub-menu a buscar... " + numero_aleatorio);

    const buttons1 = Selector('span').withText('All access');
...
Then('Verificamos que no existan errorres v17', async function () {
