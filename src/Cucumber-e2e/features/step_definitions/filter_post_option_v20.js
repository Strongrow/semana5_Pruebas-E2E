...
Given('Abrir la pagina de ghost v20', async function () {

    await testController.navigateTo(ghostPage.ghost.url());

});

When('Ingreso el usuario v20 {string}', async function (username) {
    await testController.typeText(ghostPage.ghost.loginUsername(), username);
});

When('Ingreso el password v20 {string}', async function (password) {
    await testController.typeText(ghostPage.ghost.loginPassword(), password);
});

When('Dar click en el boton Login v20', async function () {
    await testController.click(ghostPage.ghost.loginButton());
});


When('Ir a post v20', async function () {
...
When('Seleccionar un filtro al azar de los tipos de post Newest first', async function () {
    var numero_aleatorio = faker.number.int({ min: 0, max: 2 });

    console.log("valor del sub-menu a buscar... " + numero_aleatorio);

    const buttons1 = Selector('span').withText('Newest first');
...
Then('Verificamos que no existan errorres v20', async function () {
