...
Given('Abrir la pagina de ghost v12', async function () {
    await testController.navigateTo(ghostPage.ghost.url());

});

When('Ingreso el usuario v12 {string}', async function (username) {

    await testController.typeText(ghostPage.ghost.loginUsername(), username);
});

When('Ingreso el password v12 {string}', async function (password) {

    await testController.typeText(ghostPage.ghost.loginPassword(), password);

});

When('Dar click en el boton Login v12', async function () {
...
When('Ir al menu profile v12', async function () {
...
When('Realiza la busqueda con las opciones del submenu membership v12', async function () {
    let locator = Selector('input').nth(0);
    var x=['Membership','Access','Portal','Tiers','Embeddable signup form','Recommendations','Analytics'];
    var numero_aleatorio = faker.number.int({ min: 0, max: 6 });
...
Then('Verificar que no exista error al buscar v12', async function () {
